...
#pragma warning restore 0162
			if (protocol == ConnectionProtocol.WebSocket || protocol == ConnectionProtocol.WebSocketSecure) {
            	...
            	this.SocketImplementation =
Type.GetType("ExitGames.Client.Photon.SocketWebTcp, Assembly-CSharp");// typeof(SocketWebTcp);
			}
#endif
        }
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpAuthenticate()");
            ...
            if (authValues != null)
            {
                if (!string.IsNullOrEmpty(authValues.UserId)) opParameters[ParameterCode.UserId] = authValues.UserId;
            ...
            return OpCustom(ChatOperationCode.Authenticate, opParameters, true, 0, IsEncryptionAvailable);
        }
        ...
        private string GetNameServerAddress()
        {
            ...
            var currentProtocol = UsedProtocol;
            var protocolPort = 0;
            ProtocolToNameServerPort.TryGetValue(currentProtocol, out protocolPort);

            var protocolPrefix = string.Empty;
            if (currentProtocol == ConnectionProtocol.WebSocket) protocolPrefix = "ws://";
            else if (currentProtocol == ConnectionProtocol.WebSocketSecure) protocolPrefix = "wss://";
    ...
    public enum CustomAuthenticationType : byte
    {
        ...
        /// <summary>
        ///     Disables custom authentification. Same as not providing any AuthenticationValues for connect (more precisely
        ///     for: OpAuthenticate).
        /// </summary>
        None = byte.MaxValue
    }
    ...
    {
        /// <summary>
        ///     This string must contain any (http get) parameters expected by the used authentication service. By default,
        ///     username and token.
        /// </summary>
        /// <remarks>
        ///     Standard http get parameters are used here and passed on to the service that's defined in the server (Photon
        ///     Cloud Dashboard).
        /// </remarks>
        public string AuthGetParameters;

        /// <summary>
        ///     The type of custom authentication provider that should be used. Currently only "Custom" or "None" (turns this
        ///     off).
        /// </summary>
        public CustomAuthenticationType AuthType = CustomAuthenticationType.None;

        /// <summary>
        ///     After initial authentication, Photon provides a token for this client / user, which is subsequently used as
        ///     (cached) validation.
        /// </summary>
        public string Token;
        ...
        /// <summary>
        ///     Data to be passed-on to the auth service via POST. Default: null (not sent). Either string or byte[] (see
        ///     setters).
        /// </summary>
        ...
        {
            var ampersand = string.IsNullOrEmpty(AuthGetParameters) ? string.Empty : "&";
            AuthGetParameters = string.Format(
                "{0}{1}{2}={3}",
                AuthGetParameters,
                ampersand,
        ...
        public override string ToString()
        {
            return string.Format(
                "AuthenticationValues UserId: {0}, GetParameters: {1} Token available: {2}",
                UserId,
                AuthGetParameters,
                Token != null);
        }
    ...
    public class ParameterCode
    {
        ...
        /// <summary>
        ///     (214) This key's (string or byte[]) value provides parameters sent to the custom authentication service setup
        ///     in Photon Dashboard. Used in OpAuthenticate
        /// </summary>
        public const byte ClientAuthenticationData = 214;

        /// <summary>
        ///     (216) This key's (string) value provides parameters sent to the custom authentication type/service the client
        ///     connects with. Used in OpAuthenticate
        /// </summary>
    ...
    public class ErrorCode
    {
        ...
        /// <summary>
        ///     (32758) Join can fail if the room (name) is not existing (anymore). This can happen when players leave while
        ///     you join.
        /// </summary>
        ...
        /// <summary>
        ///     (-2) The operation you called is not implemented on the server (application) you connect to. Make sure you run
        ///     the fitting applications.
        /// </summary>
        public const int InvalidOperationCode = -2;

        /// <summary>
        ///     (32756) Authorization on the Photon Cloud failed because the app's subscription does not allow to use a
        ///     particular region's server.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        ///     (32757) Authorization on the Photon Cloud failed becaus the concurrent users (CCU) limit of the app's
        ...
        /// <summary>
        ///     (32760) Random matchmaking only succeeds if a room exists thats neither closed nor full. Repeat in a few
        ///     seconds or create a new room.
        /// </summary>
