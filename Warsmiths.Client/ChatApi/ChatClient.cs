...
namespace Warsmiths.Client.ChatApi
{
    ...
    ConnectionProtocol.WebSocketSecure
        ...
        {
            if (this.HasPeer &&
                (Environment.TickCount - msTimestampOfLastServiceCall > msDeltaForServiceCalls ||
            ...
            switch (statusCode)
            {
                case StatusCode.Connect:
                    ...
                    else
                    {
                        ...
                        {
                            this.didAuthenticate = this.chatPeer.AuthenticateOnNameServer(this.AppId, this.AppVersion,
                                this.chatRegion, this.AuthValues);
                            if (!this.didAuthenticate)
                            {
                                ((IPhotonPeerListener) this).DebugReturn(DebugLevel.ERROR,
        ...
        private void HandleAuthResponse(OperationResponse operationResponse)
        {
            ...
            if (operationResponse.ReturnCode == 0)
            {
                ...
                {
                    this.msDeltaForServiceCalls = this.msDeltaForServiceCalls*4;
