            ...
            PeerBase peerBase)
        {
            ...
            if (startReciept != null)
            {
                ...
                if (MasterApplication.CraftFields.ContainsKey(currentPlayer.Login))
                {
                    ...
                    {
                        if (startReciept is BaseQuest
                            && MasterApplication.CraftFields[currentPlayer.Login].Reciept.Name == startReciept.Name
                            && startReciept.Status == RecieptStatus.IsFinish)
                        {
                            _log.Debug(
                                "Its same Quest, erase last result : " + currentCharacter.CraftLevel + ":"
                                + currentCharacter.CraftExperience);
                            PlayerCommon.DicreseCraftExpririence(
                                currentPlayer,
                                currentCharacter,
                                MasterApplication.CraftFields[currentPlayer.Login].Resoult.Expereince);
                            _log.Debug(
                                "End exist receipt : " + currentCharacter.CraftLevel + ":"
                                + currentCharacter.CraftExperience);
                ...
                {
                    MasterApplication.CraftFields.Add(
                        currentPlayer.Login,
