    ...
    public class GetRecieptsHandler : BaseHandler
    {
            ...
            PeerBase peerBase)
        {
            ...
            if ((ReceptTypes)request.Type == ReceptTypes.Quest)
            {
                ...
                _log.Debug(" amount  " + newReclist.Count);
                for (var i = request.Page * maxRecieptsInPack;
                     i < newReclist.Count && recieptsInPack < maxRecieptsInPack;
                     i++)
                {
                    var questQueue = new List<BaseReciept>(
                        Warsmiths.Common.Domain.Craft.Common.CalculateQuestQuene(
                            (BaseQuest)newReclist[i],
                            MasterApplication.Reciepts)).DeepClone();

                    _log.Debug(newReclist[i].Name + ":" + i);

                    for (var z = 0; z < questQueue.Count; z++)
                    {
                        var exist = allreciepts.Find(x => x.Name == questQueue[z].Name);
                        if (exist != null)
                        {
                            questQueue[z] = exist;
                        }

                        questQueue[z].StageList = null;
                        questQueue[z].RecieptSpells = null;
                        questQueue[z].PartsPosition = null;
                        shortedReciept.Add(questQueue[z]);
                        _log.Debug(questQueue[z].Name + "added:");
                    }
                }
            ...
            {
                for (var i = request.Page * maxRecieptsInPack;
                     i < allreciepts.Count && recieptsInPack < maxRecieptsInPack;
                     i++)
                {
            ...
            var container = new RecieptsContainer { Reciepts = shortedReciept.ToArray() };
