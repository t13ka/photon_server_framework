    ...
    public class GetNextQuestHandler : BaseHandler
    {
            ...
            PeerBase peerBase)
        {
            ...
            if (!string.IsNullOrEmpty(qName))
            {
                ...
                {
                    nextQuest =
                        MasterApplication.Reciepts.Find(
                            x => x.Name == ((BaseQuest)currentQuest).NextQuest) as BaseQuest;
                ...
                else
                {
                    ...
                    var nextQuestForLevel = string.Empty;
                    if (qlvl <= currentPlayer.Characters[0].CraftLevel)
                    {
                        ...
                        if (nextQuestForLevel == qName)
                        {
                            ...
                            {
                                _log.Debug(
                                    " Maybe is  " + MasterApplication.QuestQuene[qlvl + 1] + " :lvl "
                                    + MasterApplication.QuestQuene[qlvl + 1]);
                                if (MasterApplication.GetQuestLvl(MasterApplication.QuestQuene[qlvl + 1])
                                    <= currentPlayer.Characters[0].CraftLevel)
                                {
                                    nextQuest =
                                        MasterApplication.Reciepts.Find(
                                            x => x.Name == MasterApplication.QuestQuene[qlvl + 1]);
                                }
                                else
                                {
                                    _log.Debug(" No lvlv  ");
